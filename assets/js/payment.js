...
async function setMethod(method) {
    ...
    try {
        ...
        if (data.success) {
            if (data.qrImage) openPremiumQR(data.qrImage, amount);
...
function openPremiumQR(qrData, amount) {
    ...
    modal.innerHTML = `
        <div class="bg-[#050505] w-full max-w-sm rounded-[3.5rem] p-12 text-center border border-emerald-500/20 shadow-[0_0_80px_rgba(16,185,129,0.1)] relative overflow-hidden">
            <div id="pBar" class="absolute top-0 left-0 h-[2px] bg-emerald-500 w-full"></div>
            ...
            <div class="bg-white p-6 rounded-[2.5rem] mb-8 relative group">
            ...
            <p id="timerText" class="text-xs font-black text-emerald-900 uppercase">EXPIRES IN: 05:00</p>
