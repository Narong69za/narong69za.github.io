...
// 1. ฟังก์ชันวาด Pop-up (ต้องมีเพื่อให้ระบบไม่นิ่ง)
function openPremiumQR(qrData, amount) {
    ...
    modal.innerHTML = `
        <div class="bg-[#050505] w-full max-w-sm rounded-[3.5rem] p-12 text-center border border-emerald-500/20 shadow-[0_0_80px_rgba(16,185,129,0.1)] relative overflow-hidden">
            ...
            <h2 class="text-5xl font-black italic mb-8 text-white">${amount}.00 <span class="text-xs text-emerald-900 italic">THB</span></h2>
...
async function setMethod(method) {
    ...
    try {
        ...
        if (data.success && data.qrImage) {
            if(statusEl) statusEl.innerText = "STATUS: COMPLETE";
