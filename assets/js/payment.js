...
function setMethod(method) {
    ...
    paymentBox.innerHTML = `
        <div class="glass-card p-8 rounded-3xl border border-cyan-500/20 w-full max-w-sm mx-auto text-center">
            <p class="text-[10px] font-bold text-cyan-400 uppercase mb-4">${method.toUpperCase()}</p>
            ...
            <button id="confirmBtn" class="w-full bg-cyan-500 text-white py-4 rounded-xl font-bold uppercase">ยืนยันการชำระเงิน</button>
        </div>`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if(document.getElementById("closeQr")) {
        document.getElementById("closeQr").onclick = () => { qrModal.style.display="none"; TX_LOCK=false; };
    }
});
